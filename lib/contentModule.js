...
class ContentModule extends AbstractApiModule {
  ...
  */
  async init() {
    ...
    auth.secureRoute('/api/content/', 'get', ['read:content']);
    auth.secureRoute('/api/content/', 'post', ['write:content']);
    auth.secureRoute('/api/content/', 'put', ['write:content']);
    auth.secureRoute('/api/content/', 'delete', ['write:content']);

    auth.registerAccessChecker(async (req) => {
      return true;
    });

    this.router.addHandlerMiddleware(this.schemaNameHook);

    this.setReady();
  }
  /**
  * Hook listener to store the content schemaName on client request (content works slightly differently to the standard APIs)
  * @param {ClientRequest} req
  * @param {ServerResponse} res
  * @param {Function} next
  */
  async schemaNameHook(req, res, next) {
