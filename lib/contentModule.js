...
class ContentModule extends AbstractApiModule {
  ...
  */
  async schemaNameMiddleware(req, res, next) {
    ...
    next(new Error(`Couldn't determine content type from request`));
  }
  /**
  ...
  */
  ...
  /** @override */
  async delete({ _id, _type }, ...rest) {
    // @todo deal with changes to the order of the siblings
    const mongodb = await this.app.waitForModule('mongodb');
    _id = mongodb.ObjectId.parse(_id);
    const children = await this.find({ _parentId: _id });
    const tasks = [super.delete({ _id }, ...rest)];
    if(children.length) {
      tasks.push(...children.map(c => this.delete({ _id: c._id }, ...rest)));
    }
