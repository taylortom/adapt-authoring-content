...
class ContentModule extends AbstractApiModule {
  ...
  */
  ...
  async handleValidation(originalData, data, options) { 
    ...
    let courseId;
    if(originalData) {
      courseId = originalData._type === 'course' ? originalData._id : originalData._courseId;
    } else {
      courseId = data._type === 'course' ? data._id : data._courseId;
    }
    const jsonschema = await this.app.waitForModule('jsonschema');
    const schema = await this.getSchema(options.schemaName, courseId);
  ...
  */
  async getPluginSchemas(baseSchemaName, courseId) {
    ...
    if(!config) {
      if(baseSchemaName === 'config') return [];
