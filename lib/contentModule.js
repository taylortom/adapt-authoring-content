...
class ContentModule extends AbstractApiModule {
  ...
  */
  async init() {
    ...
    this.accessCheckHook.tap(this.checkContentAccess.bind(this));
  ...
  async checkContentAccess(req, data) {
    ...
    if(data._type === 'course') {
      ...
      [course] = await this.find({ _id: data._courseId || (await this.find(data))._courseId });
    }
    const createdBy = course.createdBy && course.createdBy.toString();
    const userId = req.auth.user._id.toString();
    const shareWithUsers = course._shareWithUsers && course._shareWithUsers.map(id => id.toString()) || [];
    return course._isShared || createdBy === userId || shareWithUsers.includes(userId);
  }
  /**
  ...
  */
  ...
  /** @override */
  async delete({ _id }, ...rest) {
    // @todo deal with changes to the order of the siblings
    const mongodb = await this.app.waitForModule('mongodb');
    try { _id = mongodb.ObjectId.parse(_id); } catch {} // let the DB handle this one

    const children = await this.find({ _parentId: _id });
    const data = await Promise.all([
      super.delete({ _id }, ...rest),
      ...children.map(c => this.delete({ _id: c._id }, ...rest))
    ]);
    return children.length === 0 ? data[0] : data;
  }
  /**
