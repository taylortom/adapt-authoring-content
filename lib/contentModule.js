...
class ContentModule extends AbstractApiModule {
  ...
  */
  ...
  async handleValidation(originalData, newData, options) { 
    ...
    Object.assign(newData, jsonschema.validate(schema, newData, { useDefaults: true, ignoreRequired: false }));
  }
  ...
  */
  async serveSchema(req, res, next) {
    const { type, contentId } = req.apiData.query;
    let courseId;
    try {
      if(contentId) {
        const [item] = await this.find({ _id: contentId, _type: type });
        if(!item) {
          return res.sendError(res.StatusCodes.Error.Missing);
        }
        courseId = (type === 'course' ? item._id : item._courseId).toString();
      }
      const schema = await this.getSchema(type, courseId);
      if(!schema) {
        return res.sendError(res.StatusCodes.Error.Missing);
      }
