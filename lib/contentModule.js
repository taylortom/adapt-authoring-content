  ...
  */
  ...
  async checkContentAccess(req, data) {
    ...
    if(data._type === 'course') {
      ...
      const courseId = data._courseId || (await this.find(data))._courseId;
      [course] = await this.find({ _id: courseId });
    }
    return course._isShared || course.createdBy === req.auth.user._id;
  }
  /**
