...
class ContentModule extends AbstractApiModule {
  ...
  async insertRecursive(req) {
    ...
    if(req.apiData.query.rootId === undefined) { // new course
      childTypes.push('config');
    } else {
      parent = (await this.find({ _id: req.apiData.query.rootId }))[0];
    ...
    for(let _type of childTypes) {
      ...
      if(_type === 'course') { // also add the request body for courses
        Object.assign(data, req.apiData.data);
      }
      if(_type === 'component') {
        Object.assign(data, { 
      ...
      const item = parent = await this.insert(data, { schemaName: await this.getContentSchemaName(data) });
      newItems.push(item);
    }
