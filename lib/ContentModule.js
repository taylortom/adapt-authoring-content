...
class ContentModule extends AbstractApiModule {
  ...
  async getSchemaName(data) {
    ...
    // if _type is registered as a schema, we can return _type
    if(jsonschema.schemas[_type]) {
      return _type;
    }
    if(_id && !_type) { // no explicit type, so look for record in the DB
  ...
  async getSchema(schemaName, data) {
    ...
    const [config] = await this.find({ _type: 'config', _courseId: data._courseId }, { validate: false });
  ...
  async clone(userId, _id, _parentId, customData = {}) {
    ...
    const newData = await this.insert(Object.assign(originalDoc, { 
      _id: undefined,
      _courseId: parent?._type === 'course' ? parent?._id?.toString() : parent?._courseId?.toString(),
      _parentId: _parentId?.toString(),
      createdBy: userId.toString()
    }, customData), { schemaName });
  ...
  async updateEnabledPlugins({ _courseId, _type }) {
    ...
    const types = _enabledPlugins
      ...
      .reduce((types, pluginSchemaName) => {
        ...
        return (type === 'contentobject' ? ['menu', 'page'] : [type]).reduce((types, t) => {
          if(t && t !== 'component' && !types.includes(t)) types.push(t);
          return types;
        }, types);
      }, ['config']); // note we always need to at least update config._enabledPlugins
    // update list of enabled plugins & apply defaults
    return Promise.all(types.map(async _type => {
      const opts = { validate: false, schemaName: _type, useDefaults: true  };
      try {
        return await super.update({ _courseId, _type }, _type === 'config' ? { _enabledPlugins } : {}, opts);
      } catch(e) { // we won't necessarily have instances of all types
