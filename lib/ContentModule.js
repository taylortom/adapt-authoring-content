...
class ContentModule extends AbstractApiModule {
  ...
  async updateEnabledPlugins({ _courseId, _menu, _theme, _type }) {
    if(_type !== 'component') {
      return;
    }
    ...
    const _enabledPlugins = config._enabledPlugins.filter(p => !allComponents.includes(p)).concat(usedComponents);
