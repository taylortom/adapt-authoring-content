...
class ContentModule extends AbstractApiModule {
  ...
  async insert(data, options, mongoOptions) {
    ...
    await this.updateSortOrder(doc);
  ...
  async updateSortOrder({ _parentId, _type }) {
    if(_type === 'config' || _type === 'course' || !_parentId) {
      return;
    }
    const siblings = (await this.find({ _parentId })).sort((a, b) => a._sortOrder > b._sortOrder ? 1 : -1);
    if(!siblings.length) {
      return;
    }
  ...
  async getContentSchemaName({ _component, _id, _type }) {
    ...
    if(_id) {
      const item = (await this.find({ _id }))[0];
      if(item) {
