...
class ContentModule extends AbstractApiModule {
  ...
  async setValues() {
    ...
    /** @ignore */ this.routes = [
      ...
      {
        route: '/createcourse',
        handlers: { post: this.handleCreateCourse.bind(this) },
  ...
  async sanitiseItem(schemaName, item, options) {
    if(schemaName === this.schemaName) schemaName = await this.getContentSchemaName(item);
    return super.sanitiseItem(schemaName, item, options);
  }
  /**
  ...
  async updateEnabledPlugins({ _courseId, _type }) {
    ...
    const [[config], usedComponents] = await Promise.all([
      this.find({ _courseId, _type: 'config' }),
      this.getUsedComponentNames(_courseId)
    ]);
    if(!config) {
      return; // can't continue if there's no config to update
    }
    const _enabledPlugins = Array.from(new Set([ // generate latest list of used plugins
      ...config._enabledPlugins, 
      ...usedComponents, 
      config._menu, 
      config._theme
    ]));
    if(_enabledPlugins.length === config._enabledPlugins.length && config._enabledPlugins.every(p => _enabledPlugins.includes(p))) {
  ...
  async getUsedComponentNames(_courseId) {
    const contentplugin = await this.app.waitForModule('contentplugin');
    const plugins = (await contentplugin.find({})).map(p => p.name);
    const components = await this.find({ _type: 'component', _courseId });
    return Array.from(new Set(components.map(c => c._component).filter(c => plugins.includes(c))));
  }
  /**
  ...
  async getContentSchemaName({ _component, _id, _type }) {
    const error = this.app.errors.UNKNOWN_SCHEMA_NAME;
    let type = _type;
  ...
  /**
   * Special request handler for bootstrapping a new course with dummy content
   * @param {external:express~Request} req
   * @param {external:express~Response} res
   * @param {Function} next
   */
  async handleCreateCourse(req, res, next) {
    try {
      const createdBy = req.auth.user._id.toString();
      const course = await this.insert({ ...req.apiData.data, createdBy }, { schemaName: 'course' });
      const _courseId = course._id.toString();
      let prevItem;
      for(let _type of ['config', 'page', 'article', 'block', 'component']) {
        const data = { _type, _parentId: prevItem?._id.toString(), _courseId, createdBy };
        if(_type === 'component') {
          Object.assign(data, {
        ...
        prevItem = _type === 'config' ? course : item;
      }
