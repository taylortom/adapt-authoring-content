...
class ContentModule extends AbstractApiModule {
  ...
  async updateEnabledPlugins({ _courseId, _type }) {
    ...
    const contentplugin = await this.app.waitForModule('contentplugin');
    const allComponentNames = (await contentplugin.find({ type: 'component' }, { includeUpdateInfo: false })).map(p => p.name);
    ...
    if(_enabledPlugins.length === existingPlugins.length && _enabledPlugins.every(p => existingPlugins.includes(p))) {
      return;
    }
    // make sure the globals have been updated with the latest defaults
    try {
      const ogDoc = await this.find({ _id: _courseId });
      await this.update({ _id: _courseId }, ogDoc, { schemaName: 'course', useDefaults: true });
    } catch(e) {}
    if(_type !== 'config') {
      await this.update({ _type: 'config', _courseId }, { _enabledPlugins }, { schemaName: 'config' });
    }
  }
  /**
