...
class ContentModule extends AbstractApiModule {
  ...
  async getSchema(schemaName, data) {
    ...
    const enabledSchemas = await this.getPluginSchemas(data._courseId);
    return this.schema.getSchema(schemaName, { extensionFilter: enabledSchemas.includes });
  }
  /**
   ...
   * @return {Array<Object>}
   */
  async getPluginSchemas(_courseId) {
    const [config] = await this.find({ _type: 'config', _courseId });
    if(!config) {
      return [];
    }
    const contentplugin = await this.app.waitForModule('contentplugin');
    return config._enabledPlugins
      .reduce((m, p) => [...m, ...contentplugin.getPluginExtensionSchemas(p)], []);
  }
  /**
  ...
  async insertRecursive(req) {
    ...
    try {
      ...
      for(let _type of childTypes) {
        const data = Object.assign({ _type,  createdBy: req.auth.user._id.toString() }, defaultData[_type]);
        if(parent) {
          Object.assign(data, { 
