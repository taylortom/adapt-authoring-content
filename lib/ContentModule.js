...
class ContentModule extends AbstractApiModule {
  ...
  async init() {
    ...
    if(this.app.dependencies['adapt-authoring-authored']) {
    ...
    if(this.app.dependencies['adapt-authoring-tags']) {
      const tags = await this.app.waitForModule('tags');
      jsonschema.extendSchema('course', tags.schemaExtensionName);
    }
  ...
  async insert(data, options, mongoOptions) {
    ...
    // add the _courseId to a new course to make querying easier
    return doc._type === 'course' ? this.update({ _id: doc._id }, { _courseId: doc._id.toString() }) : doc;
  }
  /** @override */
