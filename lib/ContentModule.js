...
class ContentModule extends AbstractApiModule {
  ...
  async insert(data, options, mongoOptions) {
    ...
    if(options.updateSortOrder) await this.updateSortOrder(doc, data);
    if(options.updateEnabledPlugins) await this.updateEnabledPlugins(doc);
  ...
  async updateEnabledPlugins({ _courseId, _type }) {
    ...
    if(_enabledPlugins.length === existingPlugins.length && _enabledPlugins.every((p,i) => p === existingPlugins[i])) {
      return;
    }
