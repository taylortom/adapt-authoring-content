...
class ContentModule extends AbstractApiModule {
  ...
  async getSchema(schemaName, courseId) {
    ...
    if(courseId) {
      ...
      await Promise.all(heirarchy.map(async h => {
        const schemas = await this.getPluginSchemas(h.$anchor, courseId);
  ...
  async replace(...args) {
    // @todo deal with changes to the order of the siblings
    const doc = await super.replace(...args);
    await this.updateEnabledPlugins(doc);
    return doc;
  }
  /** @override */
  ...
  async updateEnabledPlugins({ _courseId, _type }) {
    ...
    const allComponents = (await contentplugin.find({ type: 'component' })).map(p => p.name);
    const usedComponents = (await mongodb.getCollection(this.collectionName).aggregate([
      { $match: { _type, _courseId } }, 
      { $lookup: { from: 'contentplugins', localField: '_component', foreignField: 'component', as: 'component' } },
      { $unwind: '$component' },
      { $group: { _id: '$component', data: { $addToSet: '$component' } } }
    ]).toArray()).map(p => p.data[0].name);

    const [{ _enabledPlugins: existingPlugins, _menu, _theme}] = await this.find({ _courseId, _type: 'config' });
    const _enabledPlugins = existingPlugins.filter(p => !allComponents.includes(p)).concat(usedComponents);
    // add menu/theme special cases
    if(_menu && !_enabledPlugins.includes(_menu)) _enabledPlugins.push(_menu);
    if(_theme && !_enabledPlugins.includes(_theme)) _enabledPlugins.push(_theme);

    return this.update({ _type: 'config', _courseId }, { _enabledPlugins });
  }
  /**
