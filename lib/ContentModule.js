...
class ContentModule extends AbstractApiModule {
  ...
  async updateEnabledPlugins({ _courseId, _type }) {
    ...
    const config = await contentItems.find({ _type: 'config' });

    if(!config) {
      return; // can't continue if there's no config to update
    }
    const componentNames = (contentItems.filter({ _type: 'component' })).map(c => c._component);
    ...
    const $or = _enabledPlugins
      ...
      .reduce((types, pluginSchemaName) => {
        Object.keys(jsonschema.extensionSchemas).forEach(sourceName => {
          const include = jsonschema.extensionSchemas[sourceName] === pluginSchemaName &&
    ...
    return Promise.all([ // update list of enabled plugins & apply defaults
      ...
      super.update({ _courseId, _type: { $or: Array.from($or) } }, {}, { validate: false, schemaName: 'course', useDefaults: true })
    ]);
  }
  /**
