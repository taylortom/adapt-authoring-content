...
class ContentModule extends AbstractApiModule {
  ...
  async schemaNameMiddleware(req, res, next) {
    ...
    if(reqType) {
      switch(reqType) {
        ...
        case 'menu':
          ...
          break;
        case 'component':
          req.apiData.schemaName = undefined;
          break;
        default:
          req.apiData.schemaName = reqType;
      }
      if(req.apiData.schemaName) return next();
    }
    ...
    if(item._type === 'component') {
      const contentplugin = await this.app.waitForModule('contentplugin');
      const [component] = await contentplugin.find({ name: item._component });
      if(component) req.apiData.schemaName = `${component.targetAttribute.slice(1)}-component`;
    }
    next();
  }
  /**
